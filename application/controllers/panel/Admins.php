...
class Admins extends CI_Controller {
    ...
    public function index($page = 1) {
		...
		$headerD['pageTitle'] = $headerD['settings']['pageTitle'] . " | Użytkownicy ACP";
		$this->load->view('components/Header', $headerD);

        /**  Body Section  */
        $admins = $this->AdminsM->getAll();
        ...
        $bodyD['admins'] = $this->AdminsM->getAll($start, 10);
        $config['base_url'] = base_url('panel/admins');
        $config['total_rows'] = count($admins);
        $config['per_page'] = 10;
        ...
        $this->load->view('panel/Admins', $bodyD);
    }
}